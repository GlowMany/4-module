...
exports.formatOps = formatOps;
exports.performance = performance;
exports.performanceCmp = performanceCmp;
...
function formatFixed(value) {
  ...
  return [(0, _index.formatDecimal)(a), b].join('.');
}

function formatOps(count, time) {
...
function performance(name, count, inputs, exec) {
  it(`performance: ${name}`, () => {
    const [time] = loop(count, inputs, exec);
    console.log(`
performance run for ${name} completed with ${(0, _index.formatNumber)(count)} iterations.
...
function performanceCmp(name, _ref, count, inputs, exec) {
